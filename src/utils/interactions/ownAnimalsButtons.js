...
const createAnimalsEmbeds = require('../createAnimalsEmbeds');
const Tiere = require('../../models/Tiere');

async function ownAnimalsButtons(interaction) {
  ...
  if (interaction.customId === 'ownAnimalsDown') {
    try {
      if (page != 1) {
        let newPage = +page;
        const messageData = await createAnimalsEmbeds(
          newPage - 2,
          interaction.guild.id,
          targetMessageEmbed.footer.text,
        );
    ...
    if (targetMessageEmbed.footer.text !== interaction.user.id) {
      await interaction.reply({
        content: `Du kannst nicht die Tiere anderer umbenennen!`,
        flags: MessageFlags.Ephemeral,
      });
      return;
    }
    try {
      const name = targetMessageEmbed.description
        .split('\n')[0]
        .replace('Name: ', '');
    ...
    try {
      ...
      if (!animal) {
        await interaction.reply({
          content: `Das Tier existiert nicht.`,
          flags: MessageFlags.Ephemeral,
        });
        return;
      }
      ...
      await animal.save();
      await interaction.reply({
        content: `${oldName} wurde erfolgreich in ${newName} umbenannt!`,
        flags: MessageFlags.Ephemeral,
      });
    } catch (error) {
      console.log(error);
...
module.exports = ownAnimalsButtons;
