  ...
  if (interaction.isStringSelectMenu()) {
    if (interaction.customId == 'dmselect') {
      if (interaction.member.roles.cache.some(role => role.name === interaction.values[0])) {
        await interaction.reply({ content: `Du besitzt die DM-Rolle ${interaction.values[0]} bereits.`, ephemeral: true });
        return;
      };
      ...
      await interaction.reply({ content: `Die DM-Rolle ${interaction.values[0]} wurde dir zugewiesen.`, ephemeral: true });
    }
  }
    ...
    if (interaction.customId == 'removeDm') {
      for (let i = 0; i < rolenames.length; i++) {
        if (interaction.member.roles.cache.some(role => role.name === rolenames[i])) {
          ...
          await interaction.guild.members.cache.get(interaction.member.id).roles.remove(tempRole);
          console.log(`Role ${rolenames[i]} was removed from user ${interaction.member.user.tag}`);
          await interaction.reply({ content: `Die DM-Rolle ${rolenames[i]} wurde dir entzogen.`, ephemeral: true });
          return;
        }
      }
      await interaction.reply({ content: `Du hattest gar keine DM-Rolle.`, ephemeral: true });
