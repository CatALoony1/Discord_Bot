  ...
  if (interaction.isStringSelectMenu()) {
    if (interaction.customId == 'pingselect') {
      ...
      if (addedRoles.length != 0 && removedRoles.length != 0) {
        await interaction.reply({ content: `Die Rollen ${addedRoles} wurde dir zugewiesen.\nDie Rollen ${removedRoles} wurde entfernt.`, ephemeral: true });
      } else if (addedRoles.length != 0) {
        await interaction.reply({ content: `Die Rollen ${addedRoles} wurde dir zugewiesen.`, ephemeral: true });
      } else if (removedRoles.length != 0) {
        await interaction.reply({ content: `Die Rollen ${removedRoles} wurde entfernt.`, ephemeral: true });
      } else {
    ...
    if (interaction.customId == 'removePingRoles') {
      ...
      if (removedRoles.length != 0) {
        ...
        await interaction.reply({ content: `Du hattest gar keine Ping-Rolle.`, ephemeral: true });
      }
    }
  };
};