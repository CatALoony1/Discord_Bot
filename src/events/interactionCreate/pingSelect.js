  ...
  if (interaction.isStringSelectMenu()) {
    if (interaction.customId == 'pingselect') {
      ...
      if (addedRoles.length != 0 && removedRoles.length != 0) {
        ...
        await interaction.reply({ content: `Die Rollen ${addedRoles} wurde dir zugewiesen.`, flags: MessageFlags.Ephemeral });
      } else if (removedRoles.length != 0) {
        await interaction.reply({ content: `Die Rollen ${removedRoles} wurde entfernt.`, flags: MessageFlags.Ephemeral });
      } else {
