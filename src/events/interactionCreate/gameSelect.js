  ...
  if (interaction.isStringSelectMenu()) {
    if (interaction.customId == 'gameselect') {
      ...
      if (addedRoles.length != 0 && removedRoles.length != 0) {
        await interaction.editReply(`Die Spiele ${addedRoles} wurde dir zugewiesen.\nDie Spiele ${removedRoles} wurde entfernt.`);
      } else if (addedRoles.length != 0) {
        await interaction.editReply(`Die Spiele ${addedRoles} wurde dir zugewiesen.`);
      } else if (removedRoles.length != 0) {
        await interaction.editReply(`Die Spiele ${removedRoles} wurde entfernt.`);
      } else {
    ...
    if (interaction.customId == 'removeGames') {
      ...
      if (removedRoles.length != 0) {
        ...
        await interaction.editReply(`Du hattest gar keine Spiele-Rolle.`);
      }
    }
  }
};