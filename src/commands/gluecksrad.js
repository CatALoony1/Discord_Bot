    ...
    data: new SlashCommandBuilder()
        ...
        .addIntegerOption(option =>
            option.setName('einsatz')
                .setDescription('Anzahl an Monde die du setzen möchtest.')
                .setRequired(true)
                .setMaxValue(10000)
                .setMinValue(1)
        )
    ...
    run: async ({ interaction }) => {
        ...
        try {
            ...
            if (!user || !user.bankkonto || user.bankkonto.currentMoney < einsatz) {
                interaction.editReply(`Du hast nicht genug Monde, um ${einsatz} Monde zu setzen!`);
                return;
            }
            ...
            await interaction.editReply(`Dein Einsatz in Höhe von ${einsatz} Monde wurde abgezogen!`);
            var delay = 1000;
            ...
            if (zufallsZahl <= gewinnchance) {
                if (result == einsatz) {
                    ...
                    await interaction.editReply(`Du hast deinen Einsatz von ${einsatz} Monde zurückgewonnen!\n\nGewinnchance: ${gewinnchance}% | Pool: ${gluecksrad.pool} Monde`);
                } else if (result == gluecksrad.pool) {
                    await giveMoney(targetUserObj, result);
                    await interaction.editReply(`Du hast den Jackpot geknackt und ${result} Monde gewonnen!\n\nGewinnchance: ${gewinnchance}% | Pool: ${gluecksrad.pool} Monde`);
                } else {
                    await giveMoney(targetUserObj, result);
                    await interaction.editReply(`Du hast ${result} Monde gewonnen!\n\nGewinnchance: ${gewinnchance}% | Pool: ${gluecksrad.pool} Monde`);
                }
                result = result * -1;
            } else {
                result = Math.floor(result / 2);
                await interaction.editReply(`Du hast ${result} Monde verloren!\n\nGewinnchance: ${gewinnchance}% | Pool: ${gluecksrad.pool} Monde`);
            ...
            if (sonderverlosung == 250) {
                if (gluecksrad.sonderpool != 0) {
                    ...
                    await interaction.channel.send(`Glückwunsch ${interaction.member}! Du hast bei der Sonderverlosung gewonnen und den Sonderpool von ${gluecksrad.sonderpool} Monde erhalten!`);
