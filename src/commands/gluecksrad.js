    ...
    data: new SlashCommandBuilder()
        ...
        .addIntegerOption(option =>
            option.setName('einsatz')
                .setDescription('Anzahl an Magische Kraft die du setzen möchtest.')
                .setRequired(true)
                .setMaxValue(10000)
                .setMinValue(1)
        )
    ...
    run: async ({ interaction }) => {
        ...
        try {
            ...
            if (!user || !user.bankkonto || user.bankkonto.currentMoney < einsatz) {
                interaction.editReply(`Du hast nicht genug Magische Kraft, um ${einsatz} Magische Kraft zu setzen!`);
                return;
            }
            ...
            await interaction.editReply(`Dein Einsatz in Höhe von ${einsatz} Magische Kraft wurde abgezogen!`);
            var delay = 1000;
            ...
            if (zufallsZahl <= gewinnchance) {
                if (result == einsatz) {
                    ...
                    await interaction.editReply(`Du hast deinen Einsatz von ${einsatz} Magische Kraft zurückgewonnen!\n\nGewinnchance: ${gewinnchance}% | Pool: ${gluecksrad.pool} Magische Kraft`);
                } else if (result == gluecksrad.pool) {
                    await giveMoney(targetUserObj, result);
                    await interaction.editReply(`Du hast den Jackpot geknackt und ${result} Magische Kraft gewonnen!\n\nGewinnchance: ${gewinnchance}% | Pool: ${gluecksrad.pool} Magische Kraft`);
                } else {
                    await giveMoney(targetUserObj, result);
                    await interaction.editReply(`Du hast ${result} Magische Kraft gewonnen!\n\nGewinnchance: ${gewinnchance}% | Pool: ${gluecksrad.pool} Magische Kraft`);
                }
                result = result * -1;
            } else {
                result = Math.floor(result / 2);
                await interaction.editReply(`Du hast ${result} Magische Kraft verloren!\n\nGewinnchance: ${gewinnchance}% | Pool: ${gluecksrad.pool} Magische Kraft`);
            ...
            if (sonderverlosung == 250) {
                if (gluecksrad.sonderpool != 0) {
                    ...
                    await interaction.channel.send(`Glückwunsch ${interaction.member}! Du hast bei der Sonderverlosung gewonnen und den Sonderpool von ${gluecksrad.sonderpool} Magische Kraft erhalten!`);
