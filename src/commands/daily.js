...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName('lotto').setDescription('Tägliches Lottospiel.'),
    )
    .addSubcommand((subcommand) =>
      subcommand.setName('bonus').setDescription('Täglicher Bonus.'),
    )
    .setContexts([
  ...
  run: async ({ interaction }) => {
    ...
    if (subcommand == 'lotto') {
      ...
      if (
        (user &&
          ((user.daily &&
            user.daily.toDateString() !== new Date().toDateString()) ||
            !user.daily)) ||
        !user
      ) {
        ...
        interaction.editReply(
          `Du hast deinen täglichen Bonus von ${bonusAmount} Blattläuse erhalten!`,
        );
      } else {
        interaction.editReply(
          'Du hast deinen täglichen Bonus bereits heute erhalten. Bitte versuche es morgen erneut.',
        );
      }
    }
  },
