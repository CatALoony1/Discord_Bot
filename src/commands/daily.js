    ...
    run: async ({ interaction }) => {
        ...
        if (subcommand == 'lotto') {
            ...
            if (lottozahl != 0) {
                ...
                if (anzahlNullen == 1) {
                    interaction.editReply(`Du hast diesmal nicht den Jackpot geknackt, aber du hast eine Zahl die mit einer Null endet und erhälst somit ${moneyToGive} Monde. Deine Zahl war die ${lottozahl}`);
                } else {
                    interaction.editReply(`Du hast diesmal nicht den Jackpot geknackt, aber du hast eine Zahl die mit ${anzahlNullen} Nullen endet und erhälst somit ${moneyToGive} Monde. Deine Zahl war die ${lottozahl}`);
                }
            } else {
            ...
            if ((user && ((user.daily && user.daily.toDateString() !== new Date().toDateString()) || !user.daily)) || !user) {
                ...
                interaction.editReply(`Du hast deinen täglichen Bonus von ${bonusAmount} Monde erhalten!`);
            } else {
